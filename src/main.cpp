...
int main() {
    ...
    int fieldMarginX=50;
    int fieldMarginY=50;
    int fieldWidth=windowX-(fieldMarginX*2); // fois deux à cause des deux marges
    int fieldHeight=windowY-(fieldMarginY*2); // meme chose
    // Dimensions des cages (goals)
    int goalWidth=30;
    int goalHeight=180;
    int goalY=fieldMarginY+(fieldHeight/2)-(goalHeight/2);

    InitWindow(windowX, windowY, "Ball-Terrain Complet");
    SetTargetFPS(60);
